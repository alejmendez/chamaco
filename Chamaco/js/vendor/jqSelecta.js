...
jQuery.fn.jqSelecta = function(opts){
	var thiz = this, //closure hook
		...
		defaults = {
			...
			footerCancelButtonText: 'Cancelar',
			footerOkButtonText: ' Aceptar ',
			...
			labelCallback: function(i){
				if(options.multiSelect){
					return i + ' Elemento' + ((i != 1) ? 's':'') + ' Seleccionados';
				}else{
					var s = selectOption.find("[selected='selected']");
					if(s == null){
...
//			minElementsForRender:1,  //will not render if select has less than <i>minElementsForRender</i> option elements
	...
	if(options.multiSelect === null){
		options.multiSelect = selectOption.prop('multiple');
	...
	this.refreshLabel = function(){
		if(typeof buttonText !== 'undefined'){
			buttonText.html(options.labelCallback(selectOption.find("[selected='selected']").size()));
		}
	};
	...
	this.revertState = function(){
		...
		if(currentState !== null && currentState.length > 0){
			this.selectNone();
			jQuery(currentState).attr('selected', 'selected');
		}
		this.reloadState();
		this.refreshLabel();
	};
	...
	this.reloadState = function(){
		selectOption.find("[selected='selected']").each(function(i){
	...
	this.showSelecta = function(){
		...
		currentState = selectOption.find("[selected='selected']");
		iframe = jQuery('<iframe style="border:0; overflow:hidden; position:absolute;top:0px'
	...
	this.selectInverse = function(){
		table.find('.cellectable').each(function(i){
			if(jQuery(this).data(CELL_REF).attr('selected') === 'selected'){
				deselectElement(this);
			}else{
				selectElement(this);
			}
	...
	this.getSelectedValues = function(){
		var retVal = [];
		jQuery(thiz).find("[selected='selected']").each(function(i){
	...
	function deselectElement(e){
		var c = jQuery(e);
		if(c.data(CELL_REF).prop('disabled')){return;}
	...
	function selectElement(e){
		var c = jQuery(e);
		if(c.data(CELL_REF).prop('disabled')){return;}
		c.data(CELL_REF).attr('selected', 'selected');
	...
	function toggleElement(e){
		var c = jQuery(e);
		if(c.data(CELL_REF).prop('disabled')){return;}
		if(c.data(CELL_REF).attr('selected') === 'selected' && options.multiSelect){
			deselectElement(e);
	...
	function instantiate(){
		...
		var children = selectOption.children(),
			...
			rowSize = Math.ceil(children.length / options.columns),
			row, j, i, index,
		...
		if (options.multiSelect === false && selectOption.find("[selected='selected']").size() > 0){
			singluarSelected = selectOption.find("[selected='selected']")[0].index;
			if(singluarSelected === null){
				singluarSelected = 0;
			}
		}
		//console.log((children.length / options.columns));
		...
		if(true === options.showHeader){
			if(options.showHeaderCloseIcon === true){
				...
				headerCloseIcon = jQuery('<div title="Cancelar y Cerrar" id="'+ID_BASE+'_jqselecta-header-close" class="ui-state-default ui-corner-all" style="float:right;cursor:pointer"><span class="ui-icon ui-icon-close"/></div>');
				...
				header = jQuery('<div id="'+ID_BASE+'_jqselecta-header" class="ui-widget-header ui-helper-clearfix ui-corner-top" style="padding:1px 3px"></div>');
		...
		if(options.showFooter === true){
			footer = jQuery('<div id="'+ID_BASE+'_jqselecta-footer" class="ui-widget-header ui-helper-clearfix ui-corner-bottom" style="padding:1px"></div>');
			hoverCraft = new Array();
			if(options.multiSelect === true && options.showMultiSelectFooterIcons === true){
				selectAllIcon = jQuery('<div title="Seleccionar Todo" id="'+ID_BASE+'_jqselecta-select-all" class="ui-state-default ui-corner-all" style="float:left;cursor:pointer"><span class="ui-icon ui-icon-circle-check"/></div>');
				selectNoneIcon = jQuery('<div title="No Seleccionar" id="'+ID_BASE+'_jqselecta-select-none" class="ui-state-default ui-corner-all" style="float:left;cursor:pointer"><span class="ui-icon ui-icon-circle-close"/></div>');
				selectInverseIcon = jQuery('<div title="Invertir Seleccion" id="'+ID_BASE+'_jqselecta-select-inverse" class="ui-state-default ui-corner-all" style="float:left;cursor:pointer"><span class="ui-icon ui-icon-circle-triangle-s"/></div>');
			...
			if(options.showFooterOkCancelButtons === true){
				...
				cancelButton = jQuery('<button class="ui-state-default ui-corner-all" style="text-overflow:ellipsis;overflow:hidden;float:right;cursor:pointer" type="button">'
		...
		button = jQuery('<button id="'+ID_BASE+'_jqselecta-button" class="ui-state-default ui-corner-all" style="width:'
		...
		buttonText = jQuery('<span style="width:'
				+((options.buttonWidth===null)?(selectOption.width() + 'px'):options.buttonWidth)+'text-overflow:ellipsis;overflow:hidden">'+ options.labelCallback(selectOption.find("[selected='selected']").size()) + '</span>');
		...
		selecta.wrap('<div id="'+ID_BASE+'_jqselecta-wrap" style="position:absolute;z-index:19;display:block;width:'+ options.width
				+ ((options.trace === true) ? ';border: dotted red 1px':'')
				+ ';top:'+ button.height() + 'px;left:0px'
				+ '"></div>');
