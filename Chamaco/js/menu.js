...
app.ini(function(){
    ...
    $('#deseleccionar', '#fmenuArbol').click(function(){
		$('#arbol').data('kendoTreeView').select(null);
		$('#nombre_nodo, #codigo_nodo').val('');
		return false;
	});
	
	$('#fmenuArbolElemento').submit(function(){
		return false;
	});
	
	$('#guardar_propiedad_nodo', '#fmenuArbolElemento').click(function(){
		var $datos = {
    		id 	: $('#id_nodo', '#fmenuArbolElemento').val(),
    		texto : $('#nombre_nodo', '#fmenuArbolElemento').val(),
    		codigo : $('#codigo_nodo', '#fmenuArbolElemento').val()
    	};
	...
	$('#nombre_nuevo_nodo, #codigo_nuevo_nodo', '#fmenuArbol').val('').on('keypress', function(e){
	...
	$('#nombre_nuevo_nodo', '#fmenuArbol').on('keyup', function(e){
		$('#codigo_nuevo_nodo', '#fmenuArbol').val(procesarTexto(this.value));
		this.value = ucwords(this.value);
	});
	
	$('#nombre_nodo, #codigo_nodo', '#fmenuArbolElemento').val('');
	$('#nombre_nodo', '#fmenuArbolElemento').on('keyup', function(e){
		$('#codigo_nodo', '#fmenuArbolElemento').val(procesarTexto(this.value));
		this.value = ucwords(this.value);
		return false;
	});
	...
	$('#guardar_arbol', '#fmenuArbol').click(function(e){
		e.preventDefault();
    ...
    $('#eliminar_nodo', '#fmenuArbol').click(function(e){
    	e.preventDefault();
    	var arbol_data = [];
...
function agregar_nodo(){
	...
	$nombre_nodo = $('#nombre_nuevo_nodo', '#fmenuArbol'),
	$codigo_nodo = $('#codigo_nuevo_nodo', '#fmenuArbol'),
...
function crear_arbol(r){
	ap = $('#arbol').kendoTreeView({
    	...
    	template: ' #: item.text # <input name="menu[]" type="hidden" value="#= item.id #" />',
    	dragAndDrop: true,
        animation: {
            expand: {
